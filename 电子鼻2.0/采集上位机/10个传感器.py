...
class SerialWorker(QThread):
    ...
    def __init__(self):
        ...
        self.group_counter = 0  # 【新增】文件组计数器

    def run(self):
        try:
            self.ser = serial.Serial(self.port, self.baudrate, timeout=1)
            
            # 修复：清空输入缓冲区，确保只读取连接后的最新数据
                    ...
                    if len(parts) >= 12:
                        try:
                            ...
                            if self.last_flag == 3 and flag == 0:
                                # A. 关闭旧文件并打开新文件 (修复文件同步问题)
                                self.create_new_file()     
                                # B. 通知UI清空图像
                                self.group_finished.emit() 

                            # === 2. 写入数据到文件 ===
                            # 此时 self.current_file 必定是正确的文件句柄
    ...
    def create_new_file(self):
        ...
        try:
            ...
            # 写入表头
            header = "Count,S1,S2,S3,S4,S5,S6,S7,S8,S9,S10,Flag\n"
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("多通道传感器实时采集上位机")
        self.resize(1200, 800)

        # 核心变量
        self.sensor_count = 10
        self.plot_curves = [] # 存储10条曲线对象
